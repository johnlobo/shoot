...
void update_enemy2(TEnemy2 *enemy) {
    ...
    pattern = (TPattern*) &enemy->cur_pattern;
    i = enemy->cur_cmd;
    ...
    switch (pattern->CMD) {
        ...
        case ROTATE:
        enemy->v = pattern->v;
        enemy->dtheta += pattern->theta;
        enemy->theta += pattern->theta;

        if (pattern->max < 0) {
            if (enemy->dtheta <= pattern->max) {
                enemy->dtheta = 0;
                enemy->cur_cmd++;
            }
        }
        ...
        //if (enemy->cur_cmd >= patternlength) {
